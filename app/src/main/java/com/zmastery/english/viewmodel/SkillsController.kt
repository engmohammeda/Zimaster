...
internal class SkillsController(internal val vm: AppViewModel) {
    ...
    fun evaluateWriting(text: String, promptEn: String, targetWord: String) {
        ...
        vm.vmScope.launch {
            ...
            val writer = aiAgents.firstOrNull { it.id == "writing" }
            val system = SkillsEngine.buildWritingSystem(
                targetWord = targetWord,
                promptEn = promptEn,
                level = vm.cefrEstimate.first,
                character = writer?.character.orEmpty(),
                style = writer?.style.orEmpty(),
                prompt = writer?.prompt.orEmpty(),
            )
            val res = vm.aiComplete(
                system = system,
                user = clean,
