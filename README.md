...
- مفاتيح الـ API **مشفّرة على الجهاز فقط** ولا تُرسل لأي خادم وسيط (تُستبعد حتى من النسخ الاحتياطية والمزامنة).
- نسخة الإصدار **مُعتمِدة على العمل Offline** بالكامل لحماية بيانات التعلم.
- قواعد `firestore.rules` (أعيدت كتابتها في تدقيق 2026/08/25) تضمن:
  - قراءة المحتوى العام (دروس/عبارات/إعلانات/لوحة الصدارة) للمستخدمين الموقّعين فقط.
  - كتابة الدروس والعبارات والإعلانات **للمسؤول فقط** (`role == 'admin'` في `/users`).
  - **حماية من تصعيد الصلاحيات**: لا يمكن لأي عميل كتابة `role` لنفسه — المفتاح
    يُقبل فقط من إيميل السوبر-أدمن الموثّق عبر Firebase ID Token.
  - لوحة الصدارة تقرأ من مجموعة `/leaderboard` عامة **لا تحتوي إيميلات أو أدواراً**
    (القواعد ترفض نزول أي مستند فيها يحتوي `email`/`role`).
  - تقييد كل مستخدم ببياناته الخاصة في `/users/{uid}` و `/users/{uid}/progress`.
- بعد أي تعديل على القواعد أو الفهارس: `firebase deploy --only firestore:rules,firestore:indexes`.
